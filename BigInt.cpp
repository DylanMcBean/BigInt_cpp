...
BigInt::BigInt() : numbers(1, 0), isNegative(false) {} // initialize to 0 and positive
            ...
            for (int i = 0; i < 6; i ++) {
                if (numStr[position] == '\0') {
                    ...
                    if (numStr[position] >= '0' && numStr[position] <= '9') {
                        ...
                        throw std::invalid_argument("Invalid character in number string at position " + std::to_string(position));
                        break;
                    }
                }
            }
...
BigInt BigInt::operator*(const BigInt& rhs) const {
    ...
    BigInt result;
    result.numbers = std::vector<uint32_t>(this->numbers.size() + rhs.numbers.size(), 0);
    for (size_t i = 0; i < this->numbers.size(); i++) {
        uint64_t carry = 0;
        for (size_t j = 0; j < rhs.numbers.size(); j++) {
            uint64_t product = uint64_t(this->numbers[i]) * uint64_t(rhs.numbers[j]) + carry + result.numbers[i+j];
...
const std::vector<uint32_t>& BigInt::getNumbers() const {
    return numbers;
}
...
int BigInt::numDigits() const {
  ...
  int numDigits = (numbers.size() - 1) * 6;  // Each uint32_t except the last one has up to 6 digits
